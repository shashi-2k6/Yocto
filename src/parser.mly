...
block : LBRACE block_items RBRACE { $2 }
    | LBRACE RBRACE { [] }
    ;
var_decl : ident ident EQUAL expr SEMICOLON { VariableDeclaration($1, $2, $4) }
    | ident ident SEMICOLON { VariableDeclaration($1, $2, Int(0)) }
    | ident ident EQUAL expr { VariableDeclarationExpr($1, $2, $4) }
    | ident ident { VariableDeclaration($1, $2, Int(0)) }
    ;
func_decl : FN ident LPAREN func_decl_args RPAREN block
    { 
        let params = List.map (fun stmt -> 
            ...
            | VariableDeclarationExpr(t, id, _) -> (t, id)
